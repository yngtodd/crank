...
The :code:`module/crank` subfolder contains all the :code:`*.py` files of the module.
...
This will build the :code:`*.cpp` files in the :code:`src` folder and copy the folder :code:`module/crank` folder to build location of the python module, namely :code:`${CMAKE_BINARY_DIR}/python/module/` where :code:`${CMAKE_BINARY_DIR}` is the  build directory.
...
Nevertheless we prefer to have a regular Python package with a proper :code:`__init__.py`. From the :code:`__init__.py` we import all the C++ / pybind11_ exported functionality from the build submodule named :code:`_crank`.
...
.. code-block:: cpp
    ...
    PYBIND11_MODULE(_crank , module)
    {
        ...
        crank::def_new_stuff(module);  // <- our new functionality
...
.. code-block:: python
    ...
    crank.new_stuff(numpy.arange(5), dtype='float64')
...
:code:`module/crank` subfolder.
After adding the new file, cmake needs to be rerun since we copy the content :code:`module/crank` during the build process.
